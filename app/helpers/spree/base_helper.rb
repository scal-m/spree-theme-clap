...
#  	taxon = product.taxons.taxonomy_name_equals("Instrument")
  	instr = nil
  	unless product.product_properties.empty?
			for product_property in product.product_properties
				if product_property.property.name == "instrument"
					instr = product_property.value
				end
			end
		end
		if instr
			image_tag "products/" + instr + "_off.jpg", options
  	else
  		image_tag "products/void.jpg", options
		end
	end
